...
function Disclaimer() {
  ...
  const getDisclaimer = async (page = 1) => {
    try {
      const response = await axios.get(`${API_URL}/DisclaimerRoutes/getall`, {
        headers: {
    ...
    if (isEdit) {
      try {
        const response = await axios.put(
          `${API_URL}/DisclaimerRoutes/update/${editId}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        const response = await axios.post(
          `${API_URL}/DisclaimerRoutes/create`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          <div className="custom-card card card-info card-outline mb-4">
            ...
            >
              ...
              <div className="card-footer">
                {(hasAddAccess("Disclaimer") ||
                  hasEditAccess("Disclaimer")) && (
