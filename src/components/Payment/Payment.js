...
function Payment() {
  ...
  const getOrganogram = async (page = 1) => {
    try {
      const response = await axios.get(`${API_URL}/PaymentRoutes/get/web`, {
        headers: {
      ...
      setData({
        ...
        bankDetails_en: response?.data?.data[0]?.bankDetails?.en,
        bankDetails_hi: response?.data?.data[0]?.bankDetails?.hi,
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      try {
        ...
        formData.append("bankDetails_en", data?.bankDetails_en || "");
        formData.append("bankDetails_hi", data?.bankDetails_hi || "");
        formData.append("photoTitle", data?.photoTitle || "");

        if (data?.photo) formData.append("photo", data?.photo);

        const response = await axios.put(
          `${API_URL}/PaymentRoutes/updatePayment/${editId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        ...
        Swal.fire({
          ...
          text: response.data.message || "Payment Details Successfully.",
          confirmButtonColor: "#3085d6",
        });
        getOrganogram();
      } catch (error) {
          ...
          <div className="card card-info card-outline mb-4">
            ...
            >
                  ...
                  <div className="row g-3">
                    <div>
                      ...
                      {/* <JoditEditor
                        ...
                        defaultValue={data?.bankDetails_en}
                        onBlur={(newContent) =>
                          setData((prev) => ({
                            ...prev,
              ...
              <div className="card-footer">
                {(hasAddAccess("Payment") || hasEditAccess("Payment")) && (
