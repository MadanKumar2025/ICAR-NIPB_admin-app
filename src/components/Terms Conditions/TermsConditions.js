...
function TermsConditions() {
  ...
  const getTermsConditions = async (page = 1) => {
    try {
      const response = await axios.get(
        `${API_URL}/TermsConditionsRoutes/GetAll`,
        {
          headers: {
    ...
    if (isEdit) {
      try {
        const response = await axios.put(
          `${API_URL}/TermsConditionsRoutes/update/${editId}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        const response = await axios.post(
          `${API_URL}/TermsConditionsRoutes/create`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          <div className="card card-info card-outline mb-4">
            ...
            >
              ...
              <div className="card-footer">
                {(hasAddAccess("Terms & Conditions") ||
                  hasEditAccess("Terms & Conditions")) && (
