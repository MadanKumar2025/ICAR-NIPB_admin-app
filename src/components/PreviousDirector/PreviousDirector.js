...
function PreviousDirector() {
  ...
  const handleDelete = async (item) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you really want to delete this Previous Director?",
      icon: "warning",
    ...
    try {
      ...
      await axios.delete(
        `${API_URL}/PreviousDirector/delete-previous-director/${item?.id}`,
        {
          data: {
            isActive: !item.isActive,
            updateby: decoded?.id,
      ...
      Swal.fire({
        title: "Deleted!",
        text: "Previous Director has been successfully deleted.",
        icon: "success",
        timer: 1500,
