...
function NRCPBMail() {
  ...
  const getNRCPBMail = async (page = 1) => {
    try {
      const response = await axios.get(`${API_URL}/NRCPBMailRoutes/getAll`, {
        headers: {
    ...
    if (isEdit) {
      try {
        const response = await axios.put(
          `${API_URL}/NRCPBMailRoutes/update/${editId}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        const response = await axios.post(
          `${API_URL}/NRCPBMailRoutes/create`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        <div style={{ width: "90%", marginLeft: "5%", marginTop: "3vh" }}>
          <div className="custom-card card card-info card-outline mb-4">
            <div className="card-header">
              <div className="card-title">NIPB Mail</div>
            </div>
