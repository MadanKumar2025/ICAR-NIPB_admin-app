...
function AccessibilityStatement() {
  ...
  const getAccessibilityStatement = async (page = 1) => {
    try {
      const response = await axios.get(
        `${API_URL}/AccessibilityStatementRoutes/getall`,
        {
          headers: {
    ...
    if (isEdit) {
      try {
        const response = await axios.put(
          `${API_URL}/AccessibilityStatementRoutes/update/${editId}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        await getAccessibilityStatement();
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "Error",
      ...
      try {
        const response = await axios.post(
          `${API_URL}/AccessibilityStatementRoutes/create`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          <div className="custom-card card card-info card-outline mb-4">
            ...
            >
              ...
              <div className="card-footer">
                {(hasAddAccess("Accessibility Statement") ||
                  hasEditAccess("Accessibility Statement")) && (
